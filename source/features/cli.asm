...
get_cmd:				; Main processing loop
	...
	mov di, shut_string		; 'SHUTDOWN' entered?
	call os_string_compare
	jc near shutdown

	mov di, re_string		; 'RESTART' entered?
	call os_string_compare
	jc near restart
...
restart:
	...
	jmp os_command_line		; Return if not

restart_sucess:
	mov        al, 0feh 
	out        64h, al
	jmp os_command_line			; Return if error
...
shutdown:
	...
	jmp os_command_line			; Return to Screen if not
...
continue_connection:
	...
	mov cx, 0003h
	int 15h
	jmp os_command_line			; Return to Operating System if error
...
connecterr:
	...
	mov dx, 0
	call os_dialog_box
	jmp os_command_line			; Return to Operating System
...
; ------------------------------------------------------------------
	...
	help_text		db 'Commands: DIR, COPY, REN, DEL, CAT, SIZE, CLR, HELP, TIME, DATE, VER, EXIT                               ( RESTEART ) ( SHUTDOWN )', 13, 10, 0
	...
	version_msg		db 'RGJREX ', RGJREX_VER, 13, 10, 0
	...
	shut_string		db 'SHUTDOWN', 0
	re_string		db 'RESTART', 0
